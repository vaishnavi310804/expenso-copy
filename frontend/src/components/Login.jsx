...
const Login = () => {
  ...
  const { setCurrentUserEmail } = useContext(DataContext);
  ...
  return (
    <div className={`min-h-screen flex flex-col font-sans transition-colors duration-300 ${theme ? "bg-[#0b0f19] text-white" : "bg-gray-50 text-gray-900"}`}>
      ...
      <div className="flex justify-center items-center flex-1 px-4 sm:px-6 relative overflow-hidden -mt-10">
        
        {/* Abstract Background Elements */}
        {theme ? (
          <>
            <div className="absolute top-10 right-10 w-80 h-80 bg-indigo-600/10 rounded-full blur-[100px] pointer-events-none" />
            <div className="absolute bottom-10 left-10 w-96 h-96 bg-blue-600/10 rounded-full blur-[120px] pointer-events-none" />
          </>
        ) : (
          <>
            <div className="absolute top-10 right-10 w-80 h-80 bg-indigo-200/40 rounded-full blur-[100px] pointer-events-none" />
            <div className="absolute bottom-10 left-10 w-96 h-96 bg-blue-200/40 rounded-full blur-[120px] pointer-events-none" />
          </>
        )}

        <form
          onSubmit={submitHandler}
          className={`w-full max-w-md p-8 sm:p-10 rounded-3xl shadow-2xl relative z-10 transition-all duration-300 ${
