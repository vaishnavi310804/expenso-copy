...
const ExpenseSources = () => {
  ...
  const expenseData = transactions.filter((t) => t.amount < 0).sort((a,b) => new Date(b.date) - new Date(a.date));
  ...
  return (
    <div className="p-6 sm:p-8">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 pb-4 border-b border-gray-200 dark:border-gray-800 gap-4 sm:gap-0">
        <div>
          <h2 className="text-xl font-bold tracking-tight">Expense History</h2>
          <p className={`text-sm mt-1 ${theme ? "text-gray-400" : "text-gray-500"}`}>Manage your past spending records.</p>
        </div>
        <button
          onClick={downloadCSV}
